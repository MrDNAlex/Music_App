...
public class AllPlaylistInArea extends AppCompatActivity {
    ...
    TextView PlaylistAreaName;

    ImageView playlistClick;

    CustomAdapter1 CustAdapt = new CustomAdapter1();
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_all_playlist_in_area);
        AccessSettings();
        LinearLayout AllPlaylistInAreaBackground = findViewById(R.id.AllPlaylistAreaBackground);
        PlaylistAreaName = findViewById(R.id.PlaylistAreaName2);
        ...
        PlaylistAreaName.setText(AreaName);
        Grid2.setAdapter(CustAdapt);

        if (AppAppearance.equals("Dark")) {
            AllPlaylistInAreaBackground.setBackground(ContextCompat.getDrawable(this, R.drawable.background_alex3));
            PlaylistAreaName.setTextColor(Color.rgb(255, 255, 255));
        }
    ...
    public void GetInfo() {
        ...
        AreaName = Stuff.getStringExtra("PlaylistAreaName");
        SortMethod = Stuff.getStringExtra("SortMethod");
        Position = Stuff.getIntExtra("Position", 0);
    ...
    public void ReadPlaylistInfoFile() {
        ...
        try {
            ...
            for (int i = 0; i < NumPlaylist; i++) {
                JSONArray Playlist = InfoFile.getJSONArray("PID" + i);
                for (int e = 0; e < PlaylistIDLength; e++) {
                    PlaylistID.get(e).add(Playlist.get(e).toString());
                }
            }
    ...
    public void InitializeArray() {
        ...
        for (int i = 0; i < PlaylistIDLength; i++) {
            PlaylistID.add(new ArrayList<String>());
        }

        //Initialize Array AreaID
        for (int i = 0; i < AreaIDLength; i++) {
    ...
    private class CustomAdapter1 extends BaseAdapter {
        @Override
        public int getCount() {
            return PlaylistLengthChecker.get(Position).size();
        }
        ...
        public View getView(final int position, View convertView, ViewGroup parent) {
            ...
            TextView PlaylistName = view1.findViewById(R.id.PlaylistName);
            playlistClick = view1.findViewById(R.id.playlistButton);
            ImageView PlaylistImageBar = view1.findViewById(R.id.PlaylistImageBar);

            PlaylistName.setText(PlaylistID.get(0).get(PlaylistLengthChecker.get(Position).get(position)));
            Glide.with(getApplicationContext()).load(R.drawable.dfltplayimg).placeholder(R.drawable.placeholder).into(playlistClick);
            ...
            playlistClick.setOnClickListener(new View.OnClickListener() {
                @Override
                public void onClick(View v) {
                    Intent PlaylistSongView = new Intent(getApplicationContext(), PlaylistSongView.class);
                    PlaylistSongView.putExtra("FilePath", PlaylistID.get(1).get(PlaylistLengthChecker.get(Position).get(position)));
                    startActivity(PlaylistSongView);
                }
            });
            return view1;
        }
